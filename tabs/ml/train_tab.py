...
def _build_clf(model_name: str, params: Dict[str, Any]):
    if model_name == "LogisticRegression":
        return LogisticRegression(C=float(params.get("lr_C", 1.0)), max_iter=int(params.get("lr_max_iter", 1000)))
...
def _cm_heatmap_figure(y_true: np.ndarray, y_pred: np.ndarray, labels: List[str]):
    ...
    ax.set_xticks(np.arange(len(all_labels))); ax.set_yticks(np.arange(len(all_labels)))
    ax.set_xticklabels(all_labels, rotation=45, ha="right"); ax.set_yticklabels(all_labels)
...
def make_tab():
    ...
    def on_train(
        ...
        vec_params = {"ngram_max": int(ngram_max_v), "min_df": int(min_df_v),
                      "max_df": float(max_df_v), "sublinear_tf": bool(sublinear_tf_v)}
        ...
        # Use index-based split and store it in state for others to reuse
        idx = df.index.to_numpy()
        y = df[lcol].astype(str).values
        try:
            strat = y if bool(stratify_v) else None
            idx_train, idx_test = train_test_split(
                idx, test_size=float(test_size_v),
                random_state=int(seed_v), stratify=strat
            )
        except ValueError:
            idx_train, idx_test = train_test_split(
                idx, test_size=float(test_size_v),
                random_state=int(seed_v), stratify=None
            )

        X_train = df.loc[idx_train, tcol].astype(str).values
        y_train = df.loc[idx_train, lcol].astype(str).values
        X_test  = df.loc[idx_test,  tcol].astype(str).values
        y_test  = df.loc[idx_test,  lcol].astype(str).values
        label_set = sorted(pd.Series(y).unique().tolist())

        pipe = Pipeline([("tfidf", _tfidf_vec(vec_params)), ("clf", _build_clf(model_name, clf_params))])
        ...
        st.ml_metrics = mets
        st.eval_split = {"train": [int(i) for i in idx_train], "test": [int(i) for i in idx_test]}
        st.train_test_split_config.update({
            "test_size": float(test_size_v), "random_state": int(seed_v), "stratify": bool(stratify_v),
        })
    ...
    train_event = train_btn.click(
        on_train,
        inputs=[model, ngram_max, min_df, max_df, sublinear_tf, test_size, seed, stratify,
                lr_C, lr_max_iter, svm_C, nb_alpha, rf_n_estimators, rf_max_depth],
        outputs=[metrics_json, cm_plot]
    )

    # Return handle so app.py can refresh global results after training
    return {"train_event": train_event}
