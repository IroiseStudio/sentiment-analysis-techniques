...
from langchain_huggingface import HuggingFacePipeline   # <— updated import (no deprecation)
from langchain_community.llms import HuggingFaceHub     # Hub backend (still valid)
...
def _prompt_status_text() -> str:
    ...
    bk_name = {"hf_pipeline":"Local (HF Pipeline)","hf_inference":"Hugging Face Hub","none":"—"}.get(bk, bk)
    token_note = ""
    if bk == "hf_inference":
...
def make_tab():
    ...
    def on_predict(text: str):
        ...
        try:
            ...
            return {"label": res.get("label"), "raw": res.get("raw"), "backend": res.get("backend"), "model": res.get("model"), "note": note}
    ...
    def _eval_dataset():
        ...
        st.prompt_metrics = {
            ...
            "template_len": len((getattr(st, "prompt_template", "") or "")),
        }
